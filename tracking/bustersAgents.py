...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    # find positions of remaining ghosts
    ghostLocations = []
    # ghostDistribution is an instance of util.counter (keys = pos, values = probs)
    for ghostDistribution in livingGhostPositionDistributions:
        maxProb = 0
        for pos in ghostDistribution:
            prob = ghostDistribution[pos]
            if prob > maxProb:
                maxProb = prob
                ghostLocation = pos
        ghostLocations.append(ghostLocation)

    minDist = 9999999999
    for p in ghostLocations:
        if self.distancer.getDistance(p, pacmanPosition) < minDist:
            minDist = self.distancer.getDistance(p, pacmanPosition)
            closestGhostPos = p

    # choose action to minimize distance to closestGhostPos

    minDist = 9999999999999
    for action in legal:
        p = Actions.getSuccessor(pacmanPosition, action)
        if self.distancer.getDistance(p, closestGhostPos) < minDist:
            minDist = self.distancer.getDistance(p, closestGhostPos)
            bestAction = action
